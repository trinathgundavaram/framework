...
class EventLogger:
    ...
    def batch_event(self, event_ty: str, *, req_id: int, btch_id: str, actor: str = "SYSTEM",
        ...
        self.conn.execute(
            """INSERT INTO ComplianceRequestFileDetail (Req_ID, Btch_ID, Load_ID, Ovrd_ID, Intake_ID, Trigger_ID,
                 Event_Ty, Entry_Ty, Received_File_Ref, Actor, Detail_Txt, Event_Dtts)
               VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
            (req_id, btch_id, load_id, ovrd_id, intake_id, trigger_id, event_ty, entry_ty, file_ref, actor, detail,
             self.clock.now()))
    ...
    def audit(self, event_ty: str, *, actor: str = "SYSTEM", severity: Optional[str] = None,
        ...
        self.conn.execute(
            """INSERT INTO CMS_ComplianceExceptionsAudit (Event_Ty, Event_Ctgy, Sevrty, Project_Cd, Table_Nm, Src_Cd,
                 Run_Ty, Req_ID, Load_ID, Ovrd_ID, Extract_ID, Intake_ID, Trigger_ID, Btch_ID, File_Ref, Actor,
            ...
            (event_ty, et["event_ctgy"], sev, project_cd, table_nm, src_cd, run_ty, req_id, load_id, ovrd_id,
             extract_id, intake_id, trigger_id, btch_id, file_ref, actor, self.clock.now(), description,
