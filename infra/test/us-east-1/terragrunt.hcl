##############################################################################
# test - CMS Compliance Framework metadata Glue job
#
...
# (aws/glue/test.tfvars) rather than from Terragrunt `inputs`.
##############################################################################
...
terraform {
  ...
  extra_arguments "env_tfvars" {
    ...
    arguments = ["-var-file=${get_terragrunt_dir()}/../../../aws/glue/test.tfvars"]
  }
}
