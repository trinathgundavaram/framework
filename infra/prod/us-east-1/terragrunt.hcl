##############################################################################
# prod - CMS Compliance Framework metadata Glue job
#
...
# (aws/glue/prod.tfvars) rather than from Terragrunt `inputs`.
##############################################################################
...
terraform {
  ...
  extra_arguments "env_tfvars" {
    ...
    arguments = ["-var-file=${get_terragrunt_dir()}/../../../aws/glue/prod.tfvars"]
  }
}
